...
def load_my_rebrickable_sets():
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        for set_num in set_nums:
            ...
            c.execute("SELECT COUNT(*) FROM sets WHERE set_num = ? AND status = ?", (set_num, status))
            if c.fetchone()[0] > 0:
            ...
            c.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (set_num, name, year, theme, image_url, rebrickable_url, status, datetime.now().isoformat()))
