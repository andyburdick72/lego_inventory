...
create_issue "Repository refactor: Route write endpoints" \
  --label "type:feature" --label "area:backend" --label "copilot" \
...
create_issue "Repository refactor: Error taxonomy" \
  --label "type:refactor" --label "area:backend" --label "copilot" \
...
create_issue "Repository refactor: Test coverage (phase 2a)" \
  --label "type:test" --label "area:backend" \
...
create_issue "Add Rebrickable Themes" \
  --label "type:feature" --label "area:scripts" \
...
create_issue "Pick-List Generator for a Set" \
  --label "type:feature" --label "area:scripts" --label "copilot" \
...
create_issue "Part-Out Wizard for a Set" \
  --label "type:feature" --label "area:frontend" --label "area:backend" --label "copilot" \
...
create_issue "Inventory / Set mismatch dashboard" \
  --label "type:feature" --label "area:frontend" --label "copilot" \
