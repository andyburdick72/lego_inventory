...
def test_drawers_create_rename_move_delete_and_restore(client):
    ...
    # Create new drawer
    r = client.post("/api/drawers/create", json={"name": base, "description": "Wall-1"})
    assert r.status_code in (200, 201)
    did = r.json().get("id")
    assert isinstance(did, int)

    # Duplicate by name should 409 while active
    dup = client.post("/api/drawers/create", json={"name": base})
    assert dup.status_code == 409

    # Rename
    rn = client.post("/api/drawers/rename", json={"id": did, "new_name": base + "-renamed"})
    assert rn.status_code == 200

    # Move (sort index)
    mv = client.post("/api/drawers/move", json={"id": did, "new_sort_index": 10})
    assert mv.status_code == 200

    # Delete (soft)
    dl = client.post("/api/drawers/delete", json={"id": did})
    assert dl.status_code == 200

    # Re-create with same name should RESTORE (not 409)
    r2 = client.post("/api/drawers/create", json={"name": base})
    assert r2.status_code in (200, 201)
    did2 = r2.json().get("id")
    assert isinstance(did2, int)

    # Cleanup: delete restored drawer
    client.post("/api/drawers/delete", json={"id": did2})
...
def test_containers_create_dup_move_rename_delete_and_reuse_cell(client):
    ...
    # Setup: two drawers
    d1 = client.post("/api/drawers/create", json={"name": b1})
    assert d1.status_code in (200, 201)
    drawer1 = d1.json().get("id")

    d2 = client.post("/api/drawers/create", json={"name": b2})
    ...
    cdup = client.post("/api/containers/create", json={"drawer_id": drawer1, "name": cname1})
    assert cdup.status_code == 409

    # Move to drawer2
    mv = client.post("/api/containers/move", json={"id": cid, "new_drawer_id": drawer2})
    assert mv.status_code == 200

    # Rename
    rn = client.post("/api/containers/rename", json={"id": cid, "new_name": "C2"})
    assert rn.status_code == 200

    # Delete (soft)
    dl = client.post("/api/containers/delete", json={"id": cid})
    assert dl.status_code == 200
    ...
    if isinstance(c2_id, int):
        client.post("/api/containers/delete", json={"id": c2_id})
    client.post("/api/drawers/delete", json={"id": drawer1})
    client.post("/api/drawers/delete", json={"id": drawer2})
